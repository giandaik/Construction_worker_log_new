...
// DWG MIME types vary across browsers — many send octet-stream. Extension is the reliable check.
...
export async function POST(request: Request) {
  try {
    ...
    if (!file.name.toLowerCase().endsWith('.dwg')) {
      return ApiError.badRequest('Only .dwg files are allowed');
    }
    ...
    const pathname = `projects/${projectId}/dwgs/${Date.now()}-${crypto.randomUUID()}.dwg`;
